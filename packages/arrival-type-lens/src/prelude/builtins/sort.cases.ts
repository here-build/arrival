...
expectTypeOf(__arr.sort([3, 1, 2])).toEqualTypeOf<List<number>>();
// sort with a numeric comparator; result element type preserved (number)
expectTypeOf(__arr.sort([3, 1, 2], (a: number, b: number): number => a - b)[0]).toEqualTypeOf<number>();
// comparator over string elements
expectTypeOf(__arr.sort(["b", "a"], (a: string, b: string): number => (a < b ? -1 : 1))).toEqualTypeOf<List<string>>();

// @ts-expect-error comparator must RETURN a number, not a string
__arr.sort([1, 2], (a: number, b: number): number => "nope");
// @ts-expect-error comparator params are T (number here): cannot call a string method on them
__arr.sort([1, 2], (a: number, b: number) => a.toUpperCase());
