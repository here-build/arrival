...
describe("lens rules (§4)", () => {
  ...
  it("if → ternary with the Law T guard (only #f is false)", async () => {
    expect(await p("(define (f n) (if (zero? n) 1 0))")).toContain("(n === 0) !== false ? 1 : 0");
  });

  it("if without else → guarded ternary with undefined", async () => {
    expect(await p("(define (f c) (if c 1))")).toContain("c !== false ? 1 : undefined");
  });
  ...
  it("and / not are value-returning, #f-tested (Law T) — never JS &&/!", async () => {
    expect(await p("(define (f a b) (and a (not b)))")).toContain("a === false ? false : b === false");
  });
...
describe("list layer — pair vs list (the cdr/cadr split), cut, list-ref, transpose", () => {
  ...
  it("cut → a terse lambda, one `it` param per <> slot", async () => {
    expect(await p("(define (f xs c) (filter (cut dominates? <> c) xs))")).toContain("xs.filter((it) => dominates(it, c) !== false)");
  });
  ...
  it("multi-list map with a multi-param lambda INLINES (not a broken curry)", async () => {
    ...
    expect(out).toContain("x >= ys[i] !== false ? 1 : 0");
  });
});
...
describe("let unwrapping — a let/let* body IS the arrow's block (no redundant IIFE)", () => {
  ...
  it("run-view: an infer call in a let* body unwraps so `await` sits in the async fn", async () => {
    ...
    // types/explicit-signatures: a top-level fn always carries explicit types;
    // nothing constrains `a` here, so the fallback is honest `unknown`.
    expect(out).toContain("const trace = async (a: unknown): Promise<unknown> =>");
  ...
  it("run-view: an expression-position let with an infer call awaits inline (not a sync IIFE)", async () => {
    ...
    expect(out).toContain("const f = async (c: unknown, a: unknown): Promise<"); // explicit-signatures
...
describe("arity bridge (§5)", () => {
  ...
  it("every over two lists → indexed predicate", async () => {
    ...
    expect(out).toContain("scores.every((score, i) => score >= limits[i] !== false)");
  });
...
describe("round-2 audit regressions (precedence + operators + escapes + collisions)", () => {
  ...
  it("not wraps its operand: (not (= a b)) → (a === b) === false, not a === (b === false)", async () => {
    expect(await p("(define (f a b) (not (= a b)))")).toContain("(a === b) === false");
  });
...
describe("namer heuristics — singular element + acc", () => {
  ...
  it("derives the element name from an accessor field", async () => {
    ...
    expect(out).toContain("c.scores.every((score, i) => score >= c.limits[i] !== false)");
  });
  ...
  it("derives the element name from a getter-call head: (scores a) → score", async () => {
    ...
    expect(out).toContain("scores(a).every((score, i) => score >= scores(b)[i] !== false)");
  });
  ...
  it("falls back to the role name `item` when the collection has no good singular (used whole)", async () => {
    // naming/reserved-double-underscore: every synthesized name has a named source —
    // element (plural → singular) first, then the ROLE. `__x` never reaches output.
    expect(await p("(define (f pool others) (map list pool others))")).toContain("pool.map((item, i) => [item, others[i]])");
  });
...
describe("namer — tuple destructuring + index", () => {
  ...
  it("a higher index destructures to positional names (the gepa filter shape)", async () => {
    ...
    expect(out).toContain("rows.filter(([first, second]) => (second === 0) !== false)");
  });
