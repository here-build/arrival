...
describe("keyword accessor as a first-class function", () => {
  ...
  it("the full thread-last + keyword-map + reduce pipeline projects", async () => {
    expect(await p("(define (f history) (->> history (take 3) (map :score) avg))")).toContain(
      "avg(take(3, history).map((item) => item.score))", // role-named fallback (no `__x` in output)
    );
  });
});

describe("conditionals (cond / when / unless → if, a chained ternary in both languages)", () => {
  it("`cond` with an `else` → a chained ternary (each test Law T-guarded)", async () => {
    expect(await p('(define (grade n) (cond ((>= n 90) "A") ((>= n 80) "B") (else "C")))')).toContain(
      'n >= 90 !== false ? "A" : n >= 80 !== false ? "B" : "C"',
    );
  });

  it("`cond` with no `else` falls to undefined", async () => {
    expect(await p('(define (f n) (cond ((> n 0) "pos")))')).toContain('n > 0 !== false ? "pos" : undefined');
  });

  it("`when` → a one-armed if (else undefined)", async () => {
    expect(await p("(define (f n) (when (> n 0) (log n)))")).toContain("n > 0 !== false ? log(n) : undefined");
  });

  it("`unless` negates the test through `not` (=== false), then the if-guard", async () => {
    // Double-guarded Phase 0 spelling: `not` emits the exact #f test (boolean),
    // and the `if` guard wraps it unconditionally (no type facts yet) — exact,
    // cosmetically redundant; Phase 1's facts.boolean collapses it.
    expect(await p("(define (f n) (unless (zero? n) (recip n)))")).toContain(
      "((n === 0) === false) !== false ? recip(n) : undefined",
    );
  });
  ...
  it("a `cond` reaching infer threads async through the ternary (run-view)", async () => {
    ...
    expect(out).toContain("const f = async (x: number): Promise<number> =>"); // `>` demands number; both arms are numbers
...
describe("desugar runs before async-analysis (threading/compose go async for free)", () => {
  it("a thread-last over an async fn becomes await Promise.all and the caller goes async", async () => {
    ...
    expect(out).toContain("const f = async (xs: unknown[]): Promise<unknown[]> =>"); // map demands an array; elements unconstrained
