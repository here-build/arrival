...
(define (f set)
  ...
  it("a define/overridable binding is visible to the namer, same as a plain define", () => {
    // `define/overridable` is define's macro sibling (arrival/overridable) — it only
    // expands to a real `define` at eval time, so this scanner has to recognize the
    // head literally. Before that recognition, `config/product` was absent from
    // `nameOf` entirely, exactly like the pre-W1.0 internal-define gap above.
    const m = namesByScheme(`(define/overridable config/product (s/string) "Here.build")`);
    expect(m.get("config/product")).toEqual(["configProduct"]);
  });
});
