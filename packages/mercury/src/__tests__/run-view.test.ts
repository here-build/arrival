 ...
 * whole-program shape is the `fixtures/compiled/gepa.run.ts` golden (see gepa.test.ts);
...
describe("run-view (async + ax)", () => {
  it("a fn reaching infer is async; a pure fn stays sync", async () => {
    ...
    expect(out).toContain("const ask = async (i: unknown): Promise<unknown> =>"); // explicit-signatures, honest-unknown fallback
    expect(out).toContain("const pure = (x: number): number => x + 1;");
  });
  ...
  it("a map over a `cut` of an async fn → await Promise.all, and the caller goes async", async () => {
    ...
    expect(out).toContain("const go = async (xs: unknown[]): Promise<unknown[]> =>");
  ...
  it("an internal define calling an enclosing fn-param is async (the index-map shape, Bug B)", async () => {
    ...
    expect(out).toContain("const each = async (f: unknown, xs: unknown[]): Promise<unknown[]> =>");
    expect(out).toContain("const loop = async (ys: unknown[]) =>"); // internal define: params annotated where known, no return type
