...
export interface ProjectOptions {
  ...
  /** "read" (default, sync + legible) or "run" (async + strategy-driven, runnable). */
  target?: "read" | "run";
  /** The writing strategy (design doc §2) — read ONLY when `target === "run"` (the
   *  read-view emits no runtime calls, so it has no strategy to apply). Defaults to
   *  `DEFAULT_STRATEGY` (`ts-langchain`) when `target === "run"` and this is omitted —
   *  see `assemble.ts`. */
  strategy?: Strategy;
}
