...
/**
 * The infer-family verbs as STATIC async roots (`effects/async-plane`, extended
 * per design doc §2.1): a compiled `(infer …)` is an awaited framework call, so
 * any function reaching one is async. cleanName-keyed, matching the sets this
 * analysis builds. `mcp/call`/`mcp/list` join when their lowering lands (W4) —
 * an async root without an awaited emission would leak a bare Promise.
 */
const INFER_VERB_ROOTS = new Set(["infer", "inferChat", "inferAgenticEndToEnd"]);
...
export function computeAsyncNames(forest: Node[], inferReqs: Set<string>): Set<string> {
  ...
  const visitForm = (n: Node, enclosing: Set<string>): void => {
    ...
    if (h === "let" || h === "let*") {
      ...
      if (named) {
        // The loop is a function — but its OWN name is deliberately NOT in its
        // param set: self-recursion is not an async source (that would make every
        // pure named let async). If the body reaches a real async name, the
        // fixpoint taints the loop through that name like any other function.
        register(cleanName((n.list[1] as Atom).atom), inner, body);
      }
...
function callsAsync(forms: Node[], params: Set<string>, asyncNames: Set<string>, inferReqs: Set<string>): boolean {
  const isAsync = (name: string): boolean =>
    asyncNames.has(name) || inferReqs.has(name) || params.has(name) || INFER_VERB_ROOTS.has(name);
  let found = false;
