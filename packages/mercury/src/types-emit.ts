...
function emitIf(n: ListNode, ctx: Ctx): void {
  const start = ctx.buf.offset;
  const [, c, a, b] = n.list;
  // Law T (truthiness), type-side v1: wrap EVERY condition in `__scmTruth`
  // (`(x: unknown) => boolean`, declared in the lens prelude — types.d.ts).
  // The hazard is NOT ternary type-folding (tsc doesn't fold plain ternaries);
  // it's truthiness NARROWING of arm references to the tested value —
  // `(if x x 'fallback)` would drop Scheme-truthy `0`/`""` from the true arm's
  // type. The plain-boolean wrapper blocks all condition-value narrowing; the
  // wrap stays unconditional until the §5.3 narrowing-form grammar (registry-
  // gated `narrows` flags) lands.
