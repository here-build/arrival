 ...
 *  `toJS` lowers `(list)`/`'()` to a JS `[]`, so an empty-list param element
 ...
 * boundary — `toJS(v, {})` lowers every scheme list to a JS array):
 ...
 * scheme value without the `toJS` pass (an empty list then arrives as a `Nil`
