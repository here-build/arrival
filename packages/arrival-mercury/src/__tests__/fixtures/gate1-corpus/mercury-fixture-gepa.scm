...
;; measurement). Source: inhuman/public-packages/mercury/src/__tests__/fixtures/
;; sources/gepa.scm (read-only origin package; copied, not imported — mercury
;; itself stays untouched, per the constitution's greenfield/copy-as-chunk rule
;; applied to fixtures rather than code).
;;
;; Selection rationale: the "soul" (short) GEPA variant — a genuinely different
;; implementation from inhuman-gepa-full.scm (own accessors, own control flow),
;; and the one real-program source that pairs a first-class `car` in HOF position
;; (`(map car …)`) with an independent two-list `map` zip (`(map list examples …)`)
;; in the same line — exactly the two hard cases Gate 3 pins goldens for, found
;; occurring "in the wild" rather than hand-constructed.
