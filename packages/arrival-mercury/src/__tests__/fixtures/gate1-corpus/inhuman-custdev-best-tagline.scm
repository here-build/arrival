...
;; Source: inhuman/examples/inhuman-custdev/best-tagline.scm (unmodified below the
;; rule).
;;
;; Selection rationale (R5a manifest widening — the original 3-program manifest was
;; 92% concentrated in inhuman-gepa-full.scm alone): by far the densest real program
;; found across every scouted directory this wave (examples/, mercury's fixtures/
;; sources, the oracle's own __tests__/corpus — the last is 76 one-line bug-cell
;; probes, never "real programs" in this manifest's sense). 39 car/if sites alone —
;; a recursive hill-climb with a hierarchical audience-split, worklist-fold
;; recursion, and a triage branch — more than doubling the manifest's prior total
;; denominator (38) on its own; measured via gate1/measure.ts's own
;; `measureProgram`, not eyeballed.
;;
...
(define (reaction-of-persona-tagline persona tagline)
  ((require "tagline-reaction.prompt")
    (string-append tagline "/" (:id persona))
    :summary (-> persona state-of summary-of-persona)
    :tagline tagline))
...
(define (next-tagline current reactions personas hints sys)
  (:next ((require "reflection.prompt")
...
(define (gepa-until-plateau initial personas hints sys)
  (define (loop tagline iter history)
    (let* ((reactions (reactions-of tagline personas))
           (score     (let ((n (length reactions)))
                        (if (= n 0) 0
                            (/ (count-if (compose clicking? :verdict) reactions) n))))
           (entry     (dict :tagline tagline :score score :reactions reactions))
...
(define (active-povs) (take config/pov-count (require "povs.yaml")))
...
(define (score-task task)
  (let* ((personas    (:personas task))
         ...
         (triaged     (if (>= br config/bounce-threshold)
                          ...
                          '()))
         (unsatisfied (filter (where :mismatch #f) triaged)))
...
(define (merge-initial a b)
  (:next ((require "merge.prompt") (string-append "merge-init/" a "|" b) :a a :b b)))
...
(define (consolidate-reasons label entries)
  (cond
    ...
    (else
     ((require "consolidation.prompt")
       (string-append "consolidate/" label "/" (string-concat "" (map :id entries)))
       :label    label
       :reasons  (map (lambda (e) (dict :persona (:id e) :reason (:reason e)))
                   entries)))))

;; ── entry ────────────────────────────────────────────────────────────
(define initial-personas (values-of (require "personas.yaml")))
...
(dict
  ...
  :summaries (dict
                :audience-miss (consolidate-reasons
                                  ...
                                  (filter (where :bucket "audience-miss") buckets))
                :unreachable   (consolidate-reasons
                                  "bouncing on every tagline we tried"
                                  (filter (where :bucket "unreachable") buckets))))
