 ...
 *
 * The singularize gate (`Method(_, "map", [Arrow, …])`, first param, FRESH
 * origin) replicates the dissolved legibility/singularize.ts's own trigger
 * exactly — same scope (`.map` only; `.filter`/`.reduce`/… untouched, matching
 * that leg's documented "deliberately not landed" boundary). The destructure
 * analysis (`analyzeParam`/`cdrOffsetOf`) is a verbatim port of the dissolved
 * legibility/destructure.ts's own analysis — unchanged logic, now a pure
 * census READ instead of a decide-and-rewrite pass.
 */
import { elementNameOf } from "../legibility/names.js";
import { childrenOf } from "../legibility/tree.js";
import type { Binding, CompilationUnit, Decl, Param, Pattern, R } from "../residual/types.js";
import { originOf } from "./origin.js";
import type { BindingCensus, BindingSite, DestructureShape, EntityKind } from "./types.js";
...
function registerSite(
  ...
  extra?: Partial<Pick<BindingSite, "destructure" | "singularName">>,
): void {
 ...
 *  is analyzed per-param, over the scope the param actually lives in). `first`
 *  optionally overrides the FIRST Binding-shaped param's kind/singularize
 *  candidate — the singularize gate's entry point (see `walkR`'s Method case). */
function registerParams(
  params: readonly Param[],
  body: R,
  scope: Builder,
  bySite: Map<Binding, BindingSite>,
  first?: { readonly kind: EntityKind; readonly singularName: string | undefined },
): void {
  params.forEach((p, i) => {
    ...
    const override = i === 0 ? first : undefined;
    // Destructure takes precedence over singularize — a destructured param has
    // no single remaining Binding to rename (matches the dissolved legs'
    // destructure-before-singularize ordering, legibility.ts's own doc).
    const kind = destructure !== undefined ? "param" : (override?.kind ?? "param");
    const singularName = destructure !== undefined ? undefined : override?.singularName;
    registerSite(p.pattern, kind, scope, bySite, { destructure, singularName });
  });
}
...
function walkR(n: R, scope: Builder, bySite: Map<Binding, BindingSite>): void {
  switch (n.t) {
    ...
    case "Method": {
      ...
      // trigger structurally: a `.map` call whose first arg is an Arrow whose
      // sole/leading Binding-shaped param is FRESH-minted (never a user's own
      // choice — see origin.ts's header).
      if (n.name === "map" && n.args.length > 0 && n.args[0]!.t === "Arrow") {
        const arrow = n.args[0]!;
        const firstParam = arrow.params[0];
        const isFreshFirst =
          firstParam !== undefined && firstParam.pattern.t === "Binding" && originOf(firstParam.pattern)?.mint === "fresh";
        const singularName = isFreshFirst ? elementNameOf(n.recv) : undefined;
        const child = newChildScope(scope);
        registerParams(
          arrow.params,
          arrow.body,
...
export { analyzeParam, cdrOffsetOf };
