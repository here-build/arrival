 ...
 *  replacement) and E1c's asyncness materialization (engine plan §2 E1c —
 *  the dissolved `async-ify/` pass's replacement). See census.ts for the
 ...
 *  pipeline runs it one phase after LEGIBILITY), imports.ts for the
 *  RuntimeRef→Ref commit the real pipeline drives one phase after THAT (see
 *  imports.ts's header for why the ordering survives asyncIfy's dissolution). */
