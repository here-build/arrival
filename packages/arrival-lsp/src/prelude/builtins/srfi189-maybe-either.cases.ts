...
// the result is an exact brand (callback-typed map results, SNum-param binds,
// explicit-type-arg swaps, SBool predicates) a single `.toEqualTypeOf<T>()` holds.
//
...
expectTypeOf(__arr.just(1)).toExtend<readonly ["just", SNum]>();
expectTypeOf(__arr.just(1)).not.toBeAny();
expectTypeOf(__arr.nothing()).toEqualTypeOf<readonly ["nothing"]>();
expectTypeOf(__arr.left("err")).toExtend<readonly ["left", SStr]>();
expectTypeOf(__arr.left("err")).not.toBeAny();
expectTypeOf(__arr.right(42)).toExtend<readonly ["right", SNum]>();
expectTypeOf(__arr.right(42)).not.toBeAny();

// tag predicates accept any value, return SBool
expectTypeOf(__arr["just?"](__arr.just(1))).toEqualTypeOf<SBool>();
expectTypeOf(__arr["maybe?"]("anything")).toEqualTypeOf<SBool>();
expectTypeOf(__arr["right?"](__arr.right(1))).toEqualTypeOf<SBool>();

// maybe-map threads the wrapped type through the callback (T → B) → exact brand
expectTypeOf(__arr["maybe-map"]((x: SNum): SStr => `${x}`, __arr.just(1))).toEqualTypeOf<
  readonly ["just", SStr] | readonly ["nothing"]
>();
// maybe-bind: function returns a Maybe; result unions with Nothing (SNum bind → exact)
expectTypeOf(__arr["maybe-bind"](__arr.just(1), (x: SNum) => __arr.just(x))).toEqualTypeOf<
  readonly ["just", SNum] | readonly ["nothing"]
>();
// maybe-ref unwraps to the wrapped value type (literal through T)
expectTypeOf(__arr["maybe-ref"](__arr.just(7))).toExtend<SNum>();
expectTypeOf(__arr["maybe-ref"](__arr.just(7))).not.toBeAny();
// maybe-ref/default honest union of value | default
expectTypeOf(__arr["maybe-ref/default"](__arr.just(7), "fallback")).toExtend<SNum | SStr>();
...
expectTypeOf(
  __arr["either-map"]((x: SNum): SBool => x > 0, __arr.right(1) as readonly ["left", SStr] | readonly ["right", SNum]),
).toEqualTypeOf<readonly ["left", SStr] | readonly ["right", SBool]>();
// either-ref unwraps the Right value type (literal through R)
expectTypeOf(__arr["either-ref"](__arr.right(5))).toExtend<SNum>();
...
expectTypeOf(__arr["either-swap"]<SStr, SNum>(__arr.left("x"))).toEqualTypeOf<
  readonly ["right", SStr] | readonly ["left", SNum]
>();
// either->list collects the Right payload (literal through R)
expectTypeOf(__arr["either->list"](__arr.right(9))).toExtend<List<SNum>>();
expectTypeOf(__arr["either->list"](__arr.right(9))).not.toBeAny();

// @ts-expect-error just's payload type is captured: a Just<SNum> is not assignable to Just<SStr>
const bad: readonly ["just", SStr] = __arr.just(1);
// @ts-expect-error maybe-map callback param must match the wrapped element type (SStr param over Just<SNum>)
__arr["maybe-map"]((x: SStr): SStr => x, __arr.just(1));
// @ts-expect-error maybe-ref result is the wrapped SNum, not assignable to SStr
const w: SStr = __arr["maybe-ref"](__arr.just(7));
// @ts-expect-error either-map callback must consume the Right payload type (SStr param over Right<SNum>)
__arr["either-map"]((x: SStr): SStr => x, __arr.right(1) as readonly ["left", SStr] | readonly ["right", SNum]);
// @ts-expect-error maybe->list returns a list of the wrapped type, not List<SStr>
const wl: List<SStr> = __arr["maybe->list"](__arr.just(1));
