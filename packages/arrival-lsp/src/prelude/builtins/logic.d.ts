...
// predicate chains (`(and (not (p? x)) (q? x))`) precisely SBool-compatible and
// never produces a false bite on value-flavored uses. Empty calls degenerate to
// plain SBool (T[number] of [] is never).
// ─────────────────────────────────────────────────────────────────────────────
interface ArrShape {
  and<T extends readonly unknown[]>(...xs: T): T[number] | SBool;
  or<T extends readonly unknown[]>(...xs: T): T[number] | SBool;
}
