...
expectTypeOf(__arr.every((x: number): boolean => x > 0)).toEqualTypeOf<boolean>();
// predicate over a string list
expectTypeOf(__arr.every((s: string): boolean => s.length > 0)).toEqualTypeOf<boolean>();

// @ts-expect-error predicate element type (string) mismatches the list element type (number)
__arr.every((s: string): boolean => s.length > 0);
// @ts-expect-error result is boolean, not assignable to number
const n: number = __arr.every((x: number): boolean => x > 0);
