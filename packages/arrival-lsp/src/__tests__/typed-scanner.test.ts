...
describe("narrowByType — Σ∩T at the argument slot", () => {
  ...
  it("memoizes the type query per slot — one lens call for all candidates of a step", () => {
    ...
    const scanner = narrowByType(mockScanner("argument"), { getTypeValidCandidates: spy });
    // Same slot `(car `, different trailing atoms (as candidates extend the prefix): one query.
    scanner.analyze("(car c");
    scanner.analyze("(car ca");
    scanner.analyze("(car cd");
    expect(spy).toHaveBeenCalledTimes(1);
    // A different slot → a second query.
    scanner.analyze("(+ 1 l");
