...
describe("resolveRequireProjectKey", () => {
  it("exact project-relative key", () => {
    expect(resolveRequireProjectKey(FILES, "inhuman-custdev/config.scm")).toBe(
      "inhuman-custdev/config.scm",
    );
  });

  it("strips ./ on exact key", () => {
    expect(resolveRequireProjectKey(FILES, "./inhuman-gepa/metric.scm")).toBe(
      "inhuman-gepa/metric.scm",
    );
  });
  ...
  it("lookup returns source text", () => {
    expect(
      lookupProjectFile(FILES, "config.scm", { fromFile: "inhuman-custdev/best-tagline.scm" }),
    ).toContain("config/product");
  });
});
