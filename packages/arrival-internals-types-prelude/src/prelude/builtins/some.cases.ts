...
expectTypeOf(__arr.some((x: number): boolean => x > 0)).toEqualTypeOf<boolean>();
// predicate over a string list
expectTypeOf(__arr.some((s: string): boolean => s.length > 0)).toEqualTypeOf<boolean>();

// @ts-expect-error predicate param type mismatches the list element type (string pred, number list)
__arr.some((s: string): boolean => s.length > 0);
// @ts-expect-error second arg is not a list
__arr.some((x: number): boolean => x > 0);
// @ts-expect-error result is boolean, not assignable to number
const n: number = __arr.some((x: number): boolean => x > 0);
