...
| goldenEpoch 3 (E2 exit) | 2026-07-17 | **APPROVED w/ punch-list** | V review: goldens read right, `first-class-car-hof` = the model. Conditional on the six fixes in `inhuman/docs/decisions/IN-032-mercury-behavioral-equivalence-floor.md` (punch-list retired; see git history) (export default, stdlib-over-runtime, await-elision, multi-list real-op, short-circuit static prevaluation). R-G5 (car) needs no change. |
