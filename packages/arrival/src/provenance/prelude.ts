/**
 * Q7 (PROVENANCE-PLAN.md; docs/PROVENANCE.md §1 CHOSEN round 2 A3, narrowed round 3 M1)
 ...
 * wireframe nodes; its call sites reference its template subgraph, per §1).
 *
 * THE CHECK (per define): run Q3's declaration-driven classifier (`values/lineage.ts`'s
 ...
 *
 * "Reaches a port" reads the THREE graph-layer crossing kinds §2's lowering table
 * names (`source`/`sink`/`transparent`) anywhere in the classified tree, a `fan` whose
 * mapped function itself mints (`introduces`), or an `opaque` black box — conservative:
 * an unauditable node MAY reach a port (PROVENANCE-PLAN.md Q7's own risk register:
 * "transitive port-reachability through first-class HOFs — conservative rejection is
 * correct, falls to wireframe material"). `reachesPort` is an EXHAUSTIVE structural
 * walk (not a demand-pruned cone), mirroring `countOpaqueNodes`'s traversal shape.
 *
 ...
 * merely passed as a first-class value, counts as an edge, which is exactly the
 * conservative treatment the plan's first-class-HOF risk note asks for): a define
 * referencing a port-reaching define is ITSELF port-reaching, to a fixpoint.
 */
...
export function reachesPort(n: LineageNode): boolean {
  switch (n.kind) {
    ...
    case "sink":
    case "transparent":
      return true; // the three graph-layer crossing kinds (§2's lowering table)
    case "opaque":
...
/**
 * Partition a program's top-level defines into PURE vs WIREFRAME-MATERIAL (§1 CHOSEN,
 * PROVENANCE-PLAN.md Q7). `classifier` is Q3's declaration-driven `Classifier`
 * (`values/lineage-classifier-from-env.ts`'s `classifierFromEnv(env)` in production;
 * a synthetic `Classifier` in tests). Non-define top-level forms are ignored (same
 * scope as `extract-defines.ts` — this module partitions DEFINES, not general forms).
 */
      ...
      for (const ref of refsOf.get(name) ?? []) {
        ...
        reasons.set(
          ...
          `it references "${ref}", which is wireframe material (${reasons.get(ref)}) — a define ` +
            `referencing a port-reaching define is itself port-reaching (§1's transitive closure)`,
        );
        changed = true;
        break;
      }
 ...
 * transitive) — the REJECTED half of Q7's gate (PROVENANCE-PLAN.md Q7: "a
 * fetch-wrapping helper MUST be rejected"). A no-op for a name `classifyProgramPrelude`
 * never saw (not a top-level define at all) — this door only guards MEMBERSHIP, not
 * existence.
 */
