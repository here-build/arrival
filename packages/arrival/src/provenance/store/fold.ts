/**
 * store/fold.ts — Q13: §5 C1's fold-as-recovery ("On DO wake after eviction/
 * hibernation, scope tokens, pending counters, and wrapper caches are RECONSTRUCTED
 * by folding the region's records — T7's fold law is not just a test invariant, it is
 * the recovery mechanism. In-memory region state is a cache of the stream, never the
 * source of truth"). §7's law table: "fold(events) = final region state... the SAME
...
/** Region/track state derivable ENTIRELY from a region's record stream — §5 C1's
 ...
 *  object, not a fact), and §4's silent-region/γ machinery (Q15/Q16) never needs a
...
export interface RegionFoldState {
  /** Count of `track-open` records observed — §3 I4's "started". */
  readonly started: number;
  /** Count of `track-close` records observed WITH `settled: true` — §3 I4's
  ...
  /** `started - completed` — §3 I4's completion invariant is `pending === 0` at
  ...
  /** Every `host-schedule` record observed, in stream (emission) order — §5 D5's
   ...
   *  tracks server-side (§5 D4: the store's counter is monotonic for the region's
...
/** §7's law: fold a region's ENTIRE record stream into its `RegionFoldState`. Pure —
...
export function foldRegionStream(records: readonly ProvenanceRecord[]): RegionFoldState {
  ...
  // §5 D4: "the stream's total order is EMISSION order (settlement order for async)" —
  ...
  for (const record of ordered) {
    ...
    switch (record.kind) {
      ...
      case "ingress-binding":
        // §5 C1's "region/track state" is scoped to track open/close counts + host
        // schedules (I4's completion invariant, D5's schedule) — the other four kinds
        // carry no region-lifecycle information a fold needs to recover; Q12's
        // aggregation (path-scoped RLE) and Q17's demand-lattice queries own reading
        // these, not this fold.
        break;
      default:
 ...
 *  §5 C1's "on DO wake... reconstructed by folding the region's records" describes.
 ...
 *  (id-only upsert, §5 C2/D1) with ones already durable from before the eviction that
