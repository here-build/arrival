...
// kernel assembler) — re-authored here against `createRuntimeAssembler`, the ONE assembler
// `common/kernel.ts` still hosts. Both shared the SAME underlying C3 walk
// (`common/dag-linearize.ts`'s `linearizeDag`), so the ordering laws this file pins are
// byte-identical either way — only the entry point (`.require(pack)` instead of
// `assembleEnv(env, [pack])`) changes.
...
/** The recorder's OWN env type: the kernel is env-agnostic (`createRuntimeAssembler<E>`), so
 *  this suite instantiates E as a recorder carrying its own `set` face — deliberately NOT
...
describe("schemePacks — bootstrap + wire, in dependency order", () => {
  ...
  it("evaluates bootstrap THEN runs wire for a single pack", async () => {
    ...
    await createRuntimeAssembler(env).require(pack);
  ...
  it("a dependency's bootstrap runs before its dependent's (C3 order)", async () => {
    ...
    await createRuntimeAssembler(env).require(dependent);
  ...
  it("produces a plain kernel EnvPack (composes with pure-JS packs)", async () => {
    ...
    await ra.require(jsPack);
    await ra.require(schemePack);
  ...
  it("a bootstrap-less pack is just its wire (no eval)", async () => {
    ...
    await createRuntimeAssembler(env).require(make({ name: "wire-only", wire: (e) => void e.set("x", 1) }));
