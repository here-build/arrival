// degradation.ts — door-set degradation (docs/working-proposals/symbol-define-static-program-
// validation.md §3.7, W2). Absent OPTIONAL enabling config narrows a capability's affected
// symbols to a cause-carrying DOOR-SET instead of silent withholding, when the assembling
// caller opts into `degradation: "doors"`. Two failure classes are DELIBERATELY untouched —
// present-but-invalid config (schema.parse's own job, unconditional) and pack apply errors
// (a defect, never a door) STAY throw paths in BOTH modes, unaffected by anything here.
//
...
// succeeds — arrival/infer's own known no-op-validator bug, design doc §8 decision item 9,
// untouched here). The structural check keeps a genuinely-required-but-permissive key
// (infer's) OUT of the degradable set, matching §3.7's "required config stays fail-closed"
// row — its absence is simply invisible to this module, exactly as it is invisible to
// today's `schema.parse` (a separate, already-tracked defect, not one this module should
// paper over by misclassifying it as optional).
//
...
/** Host/provisioning paths default here (today's behavior, UNCHANGED): a capability's own
 *  config-presence handling is unaffected by this mode — only a capability that explicitly
 *  consults `Activation.degradation` (via `.door(...)`) changes behavior, and only under
 *  `"doors"`. Program-scoped entry points (agent-exec, DiscoveryTool sessions, custdev
 *  harnesses) opt into `"doors"` — narrowing is caller-scoped, per §3.7's CHOSEN row. */
export type DegradationMode = "forbid" | "doors";

/** One missing input a degraded door needs to become callable — the `configuration` kind
 *  W0 shipped (`DoorCause["needs"]`'s element shape, `_bake.ts`); the `dependency`/`resource`
 *  kinds stay deferred (design doc §8 item 8, unrooted-capability policy — out of W2). */
export type DegradedNeed = DoorCause["needs"][number];

/** A capability-level entry on `AssembledEnv.degraded` (§3.7's CHOSEN "enumerable degraded
 *  list" row) — a host/discovery reader inspects this instead of inferring degradation from
 *  a throw or by probing symbols one by one. */
