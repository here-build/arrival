...
// and ignores diagnostics; `createDiagnoseLens` reads `getSemanticDiagnostics` off the SAME
// program shape. Lifting the host here keeps that compile byte-behavior-identical across
// both consumers (the decode-gate invariant).

import * as ts from "typescript";

/** Type-check one in-memory probe file against the real lib, keeping the program + checker.
 *  One call = one compile; diagnostics are read by the caller (query.ts ignores them,
 *  diagnose.ts reads them) — the compile itself is identical either way.
