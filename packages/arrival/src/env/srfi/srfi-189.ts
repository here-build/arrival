...
;; unwrap a Just; on Nothing call failure thunk (default: error)
...
;; monadic bind; Nothing short-circuits
(define (maybe-bind m f)
  (if (just? m) (f (car (cdr m))) m))
;; map over the wrapped value, preserving Nothing
...
;; unwrap a Right; on Left call failure with left value (default: error)
...
;; monadic bind; Left short-circuits
(define (either-bind e f)
  (if (right? e) (f (car (cdr e))) e))
;; map over a Right, preserving Left
