...
// base" internal callers `mintFrame(inferenceEnv, name)` a child off, bind test-only rosettas
// onto, and evaluate through `execStateOverFrame`/`execOverFrame`/`execExprOverFrame`
...
//
// POPULATION IS LAZY: `mintFrame(inferenceEnv, …)` at test setup is always safe (a frame mint is
