...
// see DEPS below) is its home. `arrival/overridable` depends on it for the runtime projection;
// the type-layer package reads the same s/* shape for the static one. Neither is more
// canonical than the other — they're two views of one proposition.
//
// ── provenance ─────────────────────────────────────────────────────────────────────────────
//
// This capability is config-free but domain-specific — the same posture `arrival/overridable`
// has — and lives outside `BASE_PACKS` (it isn't scheme stdlib). It is consumed as an ordinary
...
//
// WIRE FORMAT — the load-bearing invariant every constructor below must hold: `mercury`'s
// `type-infer.ts` (`tagToZod`) and every s/* consumer (`common/schema-tag.js`'s
...
// `("array" element)` / `("enum" value…)`. Contract enforcement is validate-ONLY: bake's
// `z.decode(def.out, …)` runs purely for its throw-on-mismatch side effect and the decoded
// value is DISCARDED — the original scheme value each body returns is handed back UNCHANGED.
// So as long as no schema below REJECTS a value a body actually produces, the wire format
// cannot drift by construction.
//
// DEPS: `cons`/`list` (scheme/lists), `pair?`/`null?` (scheme/equality), `string-append`
// (scheme/strings), `=` (scheme/numeric), `error` (scheme/r7rs/exceptions) are every
// cross-capability free name the define bodies below reach — each is a declared `deps` edge.
// `car`/`cdr` need no edge — the resolver-synth `c[ad]+r` family resolves them directly,
// unconditionally.
//
// Contract choices (mirrors srfi-128.ts's own contract-choices note):
...
//     contract just moves that failure to the call boundary with a legible message. The
//     cdr/tail stays `z.value` — genuinely unconstrained: a tag's inner shape is arbitrarily
//     recursive (an object field's type is itself a tag), and no consumer here inspects past
//     the one level `pair?`/`car` actually reads.
...
//     either to `tag` would assert a constraint this pack's own code never checks — some
//     positions ARE genuinely shapeless, and this is the legitimate use of that carve-out, not
//     a shortcut.
...
//     no body here ever produces) rather than under — the safe direction: never reject a value
//     a body actually returns.
...
//     tuple with one trailing `.optional()`) precisely to preserve the body's own tolerance for
//     being CALLED with more than one trailing element (silently ignored beyond the first,
...
export const schemaCapability = new EnvCapability("arrival/schema", {
  // See DEPS above — every cross-capability free name below resolves through a
  // declared edge, checked at bake.
