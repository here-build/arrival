...
export default new EnvCapability("scheme/strings", {
  symbols: {
    "make-string": {
      value(k: unknown, char?: unknown): SchemeString {
    ...
    string: {
      value(...chars: unknown[]): SchemeString {
    ...
    "string-length": {
      value(str: unknown): SchemeExact {
        return new SchemeExact(BigInt([...stringValue(str)].length));
      },
    },

    "string-ref": {
      value(str: unknown, k: unknown): SchemeCharacter {
    ...
    "string-contains": {
      value(str: unknown, sub: unknown): SchemeExact | boolean {
    ...
    "string-contains?": {
      value(str: unknown, sub: unknown): boolean {
        return withInputProvenance([str, sub], stringValue(str).includes(stringValue(sub)));
      },
    },

    "string-append": {
      value(...strs: unknown[]): string | SchemeString {
    ...
    "string->list": {
      value(str: unknown, start?: unknown, end?: unknown): unknown {
    ...
    "list->string": {
      value(list: unknown): SchemeString {
        const chars: string[] = [];
        let current = list;
    ...
    "string-copy": {
      value(str: unknown, start?: unknown, end?: unknown): SchemeString {
        const chars = [...stringValue(str)];
        const startIdx = start === undefined ? 0 : toIndex(start);
        const endIdx = end === undefined ? chars.length : toIndex(end);
    ...
    "string-upcase": {
      value(str: unknown): SchemeString {
    ...
    "string-map": {
      value(proc: (...args: unknown[]) => unknown, ...strings: unknown[]): string | Promise<string> {
    ...
    "string-for-each": {
      value(proc: (...args: unknown[]) => unknown, ...strings: unknown[]): void | Promise<void> {
