...
describe("conservation — every input id survives to the output or the trace", () => {
  ...
  describe("known violations — real gaps, ledgered, flip on the conservation repair", () => {
    ...
    it.fails(
      "(length (map id xs)) — the count's cone is the MINIMAL grouping fact (no per-element ids), not every element id [GATE: G2]",
      async () => {
        // MEASURED today (golden-prov-fan.test.ts, pinned green as "the A13 leak"): the
        // count's flat provenance over-attributes to EVERY element touched by map,
        // [100,101,102]. A pure-map length depends only on the collection's
        // CARDINALITY (the grouping fact), not on what each element became — the
        // static target (docs/test-suite-v2/DESIGN.md §F2) prunes the length-preserving
        // fan entirely. This fixture mints no container-level "grouping" id at all (a
        // plain `APair.fromArray` list, no Rosetta-IN crossing for the list itself), so
        // the MINIMAL correct cone here is EMPTY — asserting the absence of the leak,
        // not a specific replacement id (that id only exists once R2/the static
        // classifier's grouping-fact node lands).
  ...
  // §3 — CONTAINER-BOX ROWS, gated on the R2 ruling (open dependency, DESIGN.md §4).
