...
//
// `env as ResolvingAmbient`, not `AmbientRuntime`: `exec`'s `env` option types as
// `SchemeEnv` (V2, arrival-environment-privatization.md §II.3/D2), which the actual
// runtime value here (a `mintFrame(global_env, ...)` child, ENV T1) satisfies and plain
// `AmbientRuntime` does not.
 ...
 *
 *  ENV T1: `global_env` is a `ResolvingAmbient` (the baked-root specialization —
 *  AmbientRuntime.ts) and `mintFrame` dispatches on the parent's runtime class, so `base`
...
export async function freshEnv(): Promise<ResolvingAmbient> {
  ...
  await initBridge();
  const base = mintFrame(global_env, `test-env-${++counter}`);
  await assembleEnv(
