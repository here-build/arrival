...
export const RAMDA_FUNCTIONS = {
  ...
  fmap: R.map, // Fantasy Land alias - direct Ramda
  traverse: R.traverse,
  ...
  reduce: R.reduce,
  fold: R.reduce, // Haskell tradition
  accumulate: R.reduce, // Descriptive
  aggregate: R.reduce, // SQL-style
  ...
  // Error boundary patterns (resilience for S-expression evaluation)
  "try-prop": (key: string, obj: any) => {
    try {
      return { success: true, value: R.prop(key, obj) };
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : String(error) };
    }
  },

  "try-path": (path: (string | number)[], obj: any) => {
    try {
      return { success: true, value: R.path(path, obj) };
    } catch (error) {
      return { success: false, error: error instanceof Error ? error.message : String(error) };
    }
  },

  "try-apply": (fn: Function, ...args: any[]) => {
    try {
