...
| `budgetMs` tick + `installHeapMeter` (`heap-budget.ts`) | step-indexing / fuel | Every run terminates under bound; an unbounded loop is **contained**, not OOM. (`run-crasher.test.ts`) |
