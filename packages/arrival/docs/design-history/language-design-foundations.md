...
- [`why-no-io-dataflow-algebra.md`](./why-no-io-dataflow-algebra.md) — why the language has no IO or effects.
...
queries, hosted agent scheme runners, `.prompt` units. The secondary author is a human writing
...
3. **Quirks** — dialect-surface mismatches (JSON `true`/`false`/`null`, EDN keywords). Recover
   them; never error on a recognizable intent. (Clojure `[]`-as-vector and `{:k v}`-as-dict go
   further than recovery: they are **blessed literals** (§5), so the bracket surfaces are
   `(` … `)` lists, `[ … ]` vectors, and `{ … }` dict literals.)
...
- **Target = the R7RS-small subset.** Conformance tracked in
  [`../reference/r7rs-coverage.md`](../reference/r7rs-coverage.md); gaps are bugs to
  close, not license to invent.
- **Sandboxed, no IO, ever.** No ports, filesystem, network, process, ambient `eval`, or host
  globals. The sandbox is a *permanent* property of the base, not a mode
  ([`why-no-io-dataflow-algebra.md`](./why-no-io-dataflow-algebra.md)).
...
> **The `{}`/`[]` ruling.** SRFI-deference loses to model gravity on exactly two delimiters:
> **`{:k v}` reads as a dict literal** (≡ `(dict :k v …)`) and **`[a b c]` as a vector literal**.
> SRFI-105 curly-infix has no reader mode at all: an infix-shaped `{…}` (`{a * b}`) raises the
> teaching parse error `E-DICT-INFIX-BANNED` — never silently misparsed as a dict — and infix
> surfaces live only in the sugarcoat authoring layer. The evidence for the exception: the primary
> author is an LLM (§1), and models default to Clojure/JSON literals — a correctly-typed dict
> argument routinely arrives in several distinct wrong object encodings before a native literal is
> accepted. Meeting the model's native literal IS the zimmerframe; refusing it is the
> straightjacket. Print-back stays asymmetric (`(dict …)` / `#(…)`) — the
> literals are an input surface, not the canonical stored form. Sugarcoat is unaffected: its lens
> reads `{a + b}` as infix in its own surface and lenses `{}` declarations to `(dict …)` on the
> way in.

**The SRFI claims on the contested delimiters (the forces the ruling weighs):**
- `{ … }` is **curly-infix** (`{a + b}` → `(+ a b)`) under SRFI-105, and SRFI-110 sweet-expressions
  build on it — the strongest SRFI claim on any reserved delimiter, and the one claim the ruling
  above overrides.
- `e[ … ]` (a bracket immediately after a datum) is **neoteric bracket-apply** (`x[a]` →
  ...
  by R7RS-small — an undefended delimiter, hence blessable as the vector literal.
...
|---|---|---|
| **BLESS** | Claim as canonical input surface. Strict R7RS forms, **`#`-family** literals, and the two ruled literals. | `(list …)`, `#( … )`, `{:k v …}` dict, `[ … ]` vector |
...
- **Conservative recovery.** Fire only on genuine error/undefined; **never reinterpret a *valid*
  ...
  valid code, the exoskeleton becomes the straightjacket. (The same logic governs `[]`: the only
  acceptable states for a contested delimiter are removed outright — an unambiguous parse error —
  or *blessed* as a literal (§5). Both are unambiguous; silent reinterpretation is never one of
  them.)
- **Enumerated registry.** "As forgiving as we can" still means a *named* set of recovered idioms,
  each with its lowering and its conflict note — not open-ended guessing. Bounded and enumerated,
  like every principled superset.
...
- It emits strict R7RS or blessed-superset forms — **never bespoke sigils**. The rejected
  alternative, a `&( … )` plain-object sigil, is **non-conformant**: `&` is claimed syntax (an
  identifier character and the R6RS condition prefix) the reader can never safely round-trip.
- The plain-object form is **`(dict :k v …)`** — a plain `( … )` combination, so blessed-zone and
  non-conflicting with *every* SRFI. All three paths speak it: the serializer **emits** `(dict …)`;
  a runtime builtin **evaluates** it (companion to the `(:key d)` accessor, so
  `(:k (dict :k v))` round-trips); and it **transpiles** both ways to `{ }` / Python `{}`. Typed
  entities keep their `(TypeName …)` form. (A true-hash *datum*, if ever wanted, is Racket's
  `#`-family `#hash((k . v) …)` — but `(dict …)` is the canonical surface.)
...
|---|---|---|---|
| `[ … ]` | `(vector …)`-shaped literal, elements evaluated | **BLESS** | Vector literal, Clojure-congruent; prints back as `#(…)` (asymmetric — input surface, not stored form). (The sugarcoat `[n]`/`[:k]` subscript is a separate authoring-surface accessor, not this form.) |
| `{ … }` | `(dict :k v …)` | **BLESS** | Dict literal; prints back as `(dict …)`. Keys are keywords — JSON-style `key:` suffix spellings flip to `:key`; a bare-symbol key, odd arity, or duplicate key raises a teaching error (`E-DICT-BAD-KEY` / `E-DICT-ODD-ARITY` / `E-DICT-DUP-KEY`). An infix-shaped `{…}` raises `E-DICT-INFIX-BANNED` (§5). |
| `(dict :k v …)` | dict / object | **canonical** | Arrival's dict form — plain `( … )`, blessed-zone, round-trips via the `dict` constructor + `(:key …)` accessor, transpiles to `{ }`/Python `{}`. **The serializer emits this.** |
...
| `,` inside a `{}` / `[]` / `(dict …)` literal | position-scoped separator | **RECOVER** | At most one comma absorbed per element boundary at the positions JSON-writers emit them; elsewhere `,` stays `unquote` (quasiquote templates over the literals survive). **Never** at top level — that would steal `unquote`. |
...
clever-and-surprising trap by textbook. The §5 ruling is NOT this trap: `{}` has exactly one
meaning (dict), and an infix-shaped body is shape-*detected* only to reject it with a teaching
error — content inspection may refuse a `{}`, never reassign its meaning.

**On the canonical map.** `(dict :k v …)` beats every alternative weighed (alist, `&(…)`, `#hash`):
it is a plain `( … )` combination — homoiconic, **blessed-zone**, non-conflicting with *every* SRFI —
paired with the `(:key d)` accessor (§9). Its decisive edge over the others: the runtime
representation (object / alist / hash) is `dict`'s internal detail, invisible at the surface, which
dissolves the alist-vs-`#hash` question entirely. `{:a 1}` is a blessed *input* spelling of the same
form (§5); `(dict …)` remains the canonical stored form. Racket's `#hash((k . v) …)` stays the
`#`-family literal reserved for a *true-hash datum*; an alist is nobody's map idiom.
...
2. Which zone? **Claimed** (`&`, `,`, core `#(`) → never steal. **Blessed by ruling** (`{}` dict /
   `[]` vector, §5) → theirs; any further meaning needs model-gravity evidence of the same weight.
   **`#`-family or strict R7RS** → blessable.
